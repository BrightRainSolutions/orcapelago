...
//
// Structure of a newsletter (ground truth: docs/sightings-newsletters/):
...
// WHALES" — support both.
//
// IMPORTANT: an ALL-CAPS section banner that is not listed here is not
// recognised as a section start, so its sightings are silently swallowed into
// the preceding section — no error, no warning. The July 29, 2026 issue
// introduced NORTHERN RESIDENT KILLER WHALES and DOLPHINS, both of which were
// being dropped. Dry-run new newsletters before ingesting to catch this.
export const SPECIES_BANNERS = [
  ['SOUTHERN RESIDENTS', 'southern_resident'],
  ['NORTHERN RESIDENT KILLER WHALES', 'northern_resident'],
  ['NORTHERN RESIDENTS', 'northern_resident'],
 ...
 *   warnings: string[]                     // unrecognised section banners
 * }}
 */
export function preprocessNewsletter(rawText) {
  ...
  // An ALL-CAPS line matching no known banner, but FOLLOWED BY date headers,
  // is behaving like a section start — i.e. a species section we don't
  // recognise. Its sightings get swallowed into the preceding section and
  // stored under that section's species, silently. Warn.
  //
  // The lookahead is what keeps this quiet: ordinary boilerplate ("DONATE
  // HERE", "PHOTOS OF THE DAY") has no date headers beneath it, so it never
  // fires. Without that test every newsletter would warn on its own navigation.
  const warnings = [];
  ...
  for (let i = 0; i < footerIdx; i++) {
    ...
    if (/\d/.test(line)) continue;             // whale IDs: CRC2687, T99C, ...
    if (bannerFor(line) || /^(SUMMARY|ANNOUNCEMENTS)/i.test(line)) continue;
    if (unknownBanners.includes(line)) continue;
    // Walk forward: a date header reached WITHOUT passing a recognised banner
    // means those headers belong to this line, so it is acting as a section
    // start. Hitting a real banner first means they belong to that banner —
    // which is what promo blocks sitting just above a section look like.
    for (let j = i + 1; j < Math.min(i + 16, footerIdx); j++) {
      if (bannerFor(lines[j])) break;
      if (parseHeaderLine(lines[j])) { unknownBanners.push(line); break; }
    }
  }
  if (unknownBanners.length) {
    warnings.push(
      'unrecognised section banner(s) — sightings under these were stored under ' +
      `the preceding section's species; add to SPECIES_BANNERS: ${unknownBanners.join(' | ')}`
    );
  }
  ...
  return { title, newsletterDate, dateRange, summaryText, chunks, warnings };
}
