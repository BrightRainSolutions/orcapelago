...
/**
 * Real GNIS names that are also ordinary words, or things that are not places.
 *
 * Washington has populated places called Beach, Cove, Home, Summit, Bayview,
 * Sunset and Shoreline, bays in Clallam County called South End and North End,
 * and towns called Clipper and Triton — which are also the names of whale-watch
 * boats that reporters use as landmarks ("island side of the Clipper"). Substring
 * anchor matching fires on all of them: "south end of the marina" was handing
 * the model a bay 185 km away, and "Triton whale watching boat" a hamlet in
 * Mason County. Found 2026-09-16 by cross-checking GPS-placed sightings against
 * the place their text names; the same spurious anchors reach the model on
 * every ingest.
 *
 * Agate is here because the Mason County town shadows Agate Passage; the
 * gazetteer's own "Agate Bridge" still matches, since this list excludes exact
 * names only.
 */
const GENERIC_NAMES = new Set([
  'beach', 'cove', 'home', 'summit', 'bayview', 'view',
  'south end', 'north end', 'sunset', 'shoreline', 'clipper', 'triton', 'agate',
  // Two towns called Victor; every report saying "Victoria" was handed both.
  'victor'
]);
/**
