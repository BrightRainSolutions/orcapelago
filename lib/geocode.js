...
export async function geocodeSightings(sightings, sql, anthropic) {
  ...
  // Gazetteer entries reshaped as anchors: name + every alias, so the ugly
  // reporter phrasings we have already curated ("Lime Kiln livestream") anchor
  // too. feature_class rides along because the prompt prints it.
  const gazetteerAnchors = gazetteer.flatMap((g) => [
  ...
  if (misses.length) {
    try {
      ...
      for (let i = 0; i < misses.length; i += 60) {
        ...
        const anchors = pickAnchors(batch, [...gazetteerAnchors, ...landmarks]);
